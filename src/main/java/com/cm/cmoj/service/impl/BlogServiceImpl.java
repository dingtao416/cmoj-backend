...
import com.cm.cmoj.mapper.BlogMapper;
import com.cm.cmoj.model.dto.discuss.BlogView;
	...
	private List<BlogInfo> processBlogInfosPassword(List<BlogInfo> blogInfos) {
		for (BlogInfo blogInfo : blogInfos) {
			if (!"".equals(blogInfo.getPassword())) {
				...
				//blogInfo.setDescription(blogInfo.getDescription());
			}
			blogInfo.setTags(tagService.getTagListByBlogId(blogInfo.getId()));
		}
		return blogInfos;
...
//			throw new RuntimeException("维护博客标签关联表失败");
//		}
	...
	@Transactional(rollbackFor = Exception.class)
	@Override
	public void saveBlog(com.cm.cmoj.model.dto.discuss.Blog blog) {
	...
	@Transactional(rollbackFor = Exception.class)
	@Override
	public void updateBlog(com.cm.cmoj.model.dto.discuss.Blog blog) {
